...
import { CandidateResult, ChatMessage } from "../types";

interface ChatRequest {
  messages: ChatMessage[];
  resume_context: CandidateResult[];
}

interface UseChatReturn {
  messages: ChatMessage[];
  streaming: boolean;
  sendMessage: (userInput: string, resumeContext: CandidateResult[]) => Promise<void>;
  clearChat: () => void;
}

export function useChat(useBrowserAI: boolean): UseChatReturn {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [streaming, setStreaming] = useState<boolean>(false);

  const sendMessage = async (userInput: string, resumeContext: CandidateResult[]): Promise<void> => {
